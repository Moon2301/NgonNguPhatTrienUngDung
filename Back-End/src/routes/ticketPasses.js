...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const schema = z.object({
      ...
      passPrice: z.coerce.number().min(0), // giá / 1 vé (1 ghế)
    ...
    const booking = await col('bookings').findOne(
      { id: input.bookingId },
      { projection: { id: 1, user_id: 1, showtime_id: 1, seat_numbers: 1, total_amount: 1, status: 1 } },
    )
    ...
    const docs = []
    const ids = []
    const perSeatOriginal = Math.round(Number(booking.total_amount || 0) / Math.max(1, bookingSeats.length))
