...
export default function MyTicketsPage() {
  ...
  const fetchData = async () => {
    try {
      const [resB, resP] = await Promise.all([
        ...
        fetch(`${API_BASE}/api/ticket-passes/me`, { credentials: 'include' })
      ]);
  ...
  const handleCancelPass = async (id) => {
    if (!window.confirm('Bạn có chắc muốn ngừng rao bán ghế này?')) return
    try {
      ...
      if (res.ok) {
        fetchData()
        alert('Đã hủy rao bán thành công')
      }
    } catch (err) {
        ...
        <div className="listing-tab-content anim-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 40 }}>
           ...
           <div>
              <h4 style={{ color: '#fff', fontSize: 13, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
                 ...
                 Đang rao bán ({myPasses.filter(p => p.status === 'AVAILABLE').length})
              </h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {myPasses.filter(p => p.status === 'AVAILABLE').length === 0 ? (
                  ...
                  myPasses.filter(p => p.status === 'AVAILABLE').map(pass => (
                    <div key={pass.id} className="pele-card" style={{ display: 'flex', gap: 12, padding: 12, alignItems: 'center', background: 'rgba(30,30,35,0.6)', borderRadius: 16 }}>
                       ...
                       <button 
                          ...
                          style={{ padding: '6px 12px', background: 'rgba(255,255,255,0.05)', borderRadius: 10, color: '#ff4d4d', fontSize: 10, fontWeight: 700, flexShrink: 0 }}
                       >
                         Hủy
                       </button>
                    </div>
...
function TicketDetailModal({ ticket, onClose }) {
  ...
  const [resalePricePerSeat, setResalePricePerSeat] = useState(ticket.total_amount / (ticket.seat_numbers.split(',').length))
  ...
  const handleResaleListing = async (e) => {
    e.preventDefault()
    if (selectedSeats.length === 0) return alert('Vui lòng chọn ít nhất một ghế muốn bán')
    setListing(true)
    try {
      const res = await fetch(`${API_BASE}/api/ticket-passes`, {
        ...
        body: JSON.stringify({
          bookingId: ticket.id,
          seats: selectedSeats,
          pricePerSeat: resalePricePerSeat
        })
      })
      const data = await res.json()
      if (res.ok) {
        alert(`Đã đăng bán ${selectedSeats.length} ghế thành công!`)
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        padding: '20px 10px', overflowY: 'auto', WebkitOverflowScrolling: 'touch'
      }} 
      onClick={onClose}
    >
      <div className="ticket-modal-inner" style={{ 
        width: '100%', maxWidth: 450, background: '#111', borderRadius: '32px', 
        position: 'relative', boxShadow: '0 50px 100px rgba(0,0,0,0.8)',
        animation: 'modalSlideUp 0.4s cubic-bezier(0.16, 1, 0.3, 1)',
        margin: 'auto 0' // Căn giữa theo trục dọc nếu nội dung ngắn
        ...
        <div style={{ padding: '24px 24px 40px' }}>
          ...
          {showResaleForm ? (
            <div className="resale-form anim-fade-in" style={{ padding: 24, background: '#1a1a1a', borderRadius: 24, border: '1px solid #e50914' }}>
               ...
               <div style={{ marginBottom: 20, padding: 12, background: 'rgba(229, 9, 20, 0.1)', borderRadius: 12, border: '1px solid rgba(229, 9, 20, 0.2)' }}>
                  <p style={{ margin: 0, fontSize: 11, color: '#ff4d4d' }}>
                    ...
                    Lưu ý: Giá bán không được cao hơn giá gốc ({ticket.originalPrice?.toLocaleString()}đ)
                  </p>
               </div>
               <form onSubmit={handleResaleListing}>
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                       {ticket.seat_numbers.split(',').map(s => {
                         const isListed = ticket.listedSeats?.includes(s);
                         return (
                           <button 
                             key={s} type="button" 
                             disabled={isListed}
                             ...
                             style={{ 
                               ...
                               background: isListed ? '#333' : (selectedSeats.includes(s) ? '#e50914' : 'transparent'),
                  ...
                  <div style={{ marginBottom: 20 }}>
                     ...
                     <input 
                       ...
                       style={{ width: '100%', padding: '12px 16px', borderRadius: 12, background: '#000', border: '1px solid #333', color: '#fff', fontSize: 16 }}
                      />
                     {resalePricePerSeat > ticket.originalPrice && (
                       <p style={{ color: '#e50914', fontSize: 11, marginTop: 8 }}>Giá bán vượt quá giá gốc!</p>
                     )}
                     <p className="muted" style={{ fontSize: 11, marginTop: 8 }}>Tổng thu về: {(resalePricePerSeat * selectedSeats.length).toLocaleString()}đ</p>
                  </div>
                  <div style={{ display: 'flex', gap: 12 }}>
                     <button type="submit" disabled={listing} className="btn-primary-pele" style={{ flex: 1, padding: '12px', borderRadius: 12 }}>
                        {listing ? 'Đang đăng...' : 'Xác nhận rao bán'}
                     </button>
                     <button type="button" onClick={() => setShowResaleForm(false)} style={{ flex: 1, padding: '12px', borderRadius: 12, background: '#333', color: '#fff', border: 'none', cursor: 'pointer' }}>
                        Hủy
                     </button>
                  </div>
               </form>
          ...
          <div className="hide-on-print" style={{ marginTop: 40, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            >
              <i className="fa-solid fa-shop" style={{ marginRight: 8 }}></i> Rao bán lại vé này
            </button>
          </div>
        </div>
      </div>
